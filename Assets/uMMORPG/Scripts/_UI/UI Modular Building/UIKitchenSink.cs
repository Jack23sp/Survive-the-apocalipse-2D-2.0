...
public class UIKitchenSink : MonoBehaviour, IUIScript
{
    public static UIKitchenSink singleton;
    ...
    public bool drink;
    public List<int> waterContainer = new List<int>();

    [HideInInspector] public KitchenSink sink;
    ...
    {
        UIUtils.BalancePrefabs(drinkObjectToSpawn, waterContainer.Count, content);
        for(int i = 0; i < waterContainer.Count; i++)
        {
            int index = i;
            UIFillSlot fillSlot = content.GetChild(index).GetComponent<UIFillSlot>();
            fillSlot.actual.text = Player.localPlayer.inventory.slots[waterContainer[index]].item.waterContainer.ToString();
            fillSlot.fillButton.interactable = Player.localPlayer.inventory.slots[waterContainer[index]].item.waterContainer < ((WaterBottleItem)Player.localPlayer.inventory.slots[waterContainer[index]].item.data).maxWater;
            fillSlot.min.text = "0";
            fillSlot.max.text = ((WaterBottleItem)Player.localPlayer.inventory.slots[waterContainer[index]].item.data).maxWater.ToString();
            fillSlot.itemImage.sprite = ((WaterBottleItem)Player.localPlayer.inventory.slots[waterContainer[index]].item.data).image;
            fillSlot.itemName.text = ((WaterBottleItem)Player.localPlayer.inventory.slots[waterContainer[index]].item.data).name;
            ...
            fillSlot.fillButton.onClick.AddListener(() =>
            {
                ...
                Player.localPlayer.playerThirsty.CmdFillBottle(waterContainer[index], aquifer.netIdentity);
            });
        }
        ChangeWater();
    }
            ...
            for(int i= 0; i < Player.localPlayer.inventory.slots.Count; i++)
            {
                ...
                if (Player.localPlayer.inventory.slots[index].amount > 0)
                {
                    if (Player.localPlayer.inventory.slots[index].item.data is WaterBottleItem)
                    {
                        ...
                        {
                            if (!waterContainer.Contains(index)) waterContainer.Add(index);
                        }
                    }
                }
            }
    ...
    public void Open(KitchenSink kitchenSink)
    {
        if (Player.localPlayer)
        {
            sink = kitchenSink;
