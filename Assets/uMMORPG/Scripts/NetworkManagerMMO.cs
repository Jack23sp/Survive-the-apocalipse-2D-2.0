...
public partial class NetworkManagerMMO : NetworkManager
{
    ...
    {
        GameObject man = Instantiate(manager,Vector3.zero,Quaternion.identity);
        NetworkServer.Spawn(man);
    }
    ...
    CharactersAvailableMsg MakeCharactersAvailableMessage(string account)
    {
        ...
        characters = Database.singleton.CharactersForAccount(account);
        CharactersAvailableMsg message = new CharactersAvailableMsg();
        message.characters = new CharactersAvailableMsg.CharacterPreview[0];

        foreach (string characterName in characters)
        {
            message = Database.singleton.LoadCharacterCreationForPreview(characterName, message);
        }
    ...
    void LoadPreview(GameObject prefab, Transform location, int selectionIndex, CharactersAvailableMsg.CharacterPreview character)
    {
        ...
        PlayerCharacterCreation playerCharacterCreation = player.GetComponent<PlayerCharacterCreation>();

        playerCharacterCreation.hairType = character.hairIndex;
        playerCharacterCreation.hairColor = character.hairColor;
        playerCharacterCreation.beard = character.bearIndex;
        playerCharacterCreation.skinColor = character.skinColor;
        playerCharacterCreation.eyesColor = character.eyesColor;
        playerCharacterCreation.underwearColor = character.underwearColor;
        playerCharacterCreation.fat = character.fatValue;
        playerCharacterCreation.thin = character.thinValue;
        playerCharacterCreation.height = character.heightValue;
        playerCharacterCreation.muscle = character.muscleValue;
        playerCharacterCreation.breast = character.breastValue;
        playerCharacterCreation.sex = character.gender;
        playerCharacterCreation.hats = character.hats;
        playerCharacterCreation.accessory = character.accessory;
        playerCharacterCreation.upper = character.upper;
        playerCharacterCreation.down = character.down;
        playerCharacterCreation.shoes = character.shoes;
        playerCharacterCreation.bag = character.bag;

        playerCharacterCreation.SetupPreview(character);
    ...
    void OnClientCharactersAvailable(CharactersAvailableMsg message)
    {
        ...
        {
            int index = i;
            CharactersAvailableMsg.CharacterPreview character = charactersAvailableMsg.characters[index];

            // find the prefab for that class
            Player prefab = playerClasses.Find(p => p.name == character.className);
            if (prefab != null)
                LoadPreview(prefab.gameObject, selectionLocations[index], index, character);
            else
    ...
    Player CreateCharacter(GameObject classPrefab, string characterName, string account, CharacterCreateMsg character)//, bool gameMaster)
    {
        ...
        if (player.equipment.slots[7].amount > 0 && ((EquipmentItem)player.equipment.slots[7].item.data).indexBag > -1)
        {
            player.inventory.bagsize = ((EquipmentItem)player.equipment.slots[7].item.data).additionalSlot.Get(player.equipment.slots[7].item.bagLevel);
        }
        for(int i = 0; i < ability.networkAbilities.Count; i++)
        {
            if (ability.networkAbilities[i].name == "Explorer")
                player.inventory.abilitySize = Convert.ToInt32(ability.networkAbilities[i].level);
        }
        player.inventory.size = player.inventory.defaultSize + player.inventory.bagsize + player.inventory.abilitySize;
                ...
                if (!Database.singleton.CharacterExists(message.name))
                {
                    ...
                    if (Database.singleton.CharactersForAccount(account).Count < characterLimit)
                    {
                        ...
                        if (0 <= message.classIndex && message.classIndex < playerClasses.Count)
                        {
                            ...
                            //{
                            // create new character based on the prefab.
                            Player player = CreateCharacter(playerClasses[message.classIndex].gameObject, message.name, account, message); //, message.gameMaster);
    ...
    public override void OnServerDisconnect(NetworkConnectionToClient conn)
    {
        ...
        if (conn.identity != null)
        {
            ...
            if(player.playerModularBuilding.fakeBuildingID != null)
            {
                player.playerModularBuilding.ManageVisibilityOfObject(player.playerModularBuilding.fakeBuildingID, false);
            }
        }
