...
public class PlayerTrading : NetworkBehaviour
{
    ...
    [Header("Trading")]
    [SyncVar(hook =(nameof(CheckRequest))), HideInInspector] public string requestFrom = "";
    ...
    public void CheckRequest(string oldRequest, string newRequest)
    {
        // Active group invite
    }
    ...
    public bool CanStartTradeWith(Player entity)
    {
    ...
    public void SetRequest(NetworkIdentity identity)
    {
        Player sender = identity.GetComponent<Player>();
        // validate
        if (sender && sender.trading.CanStartTradeWith(player))
        {
            // send a trade request to target
            player.trading.requestFrom = sender.name;
