...
[CreateAssetMenu(menuName = "uMMORPG Item/Fence", order = 999)]
public partial class ScriptableFence : ScriptableBuildingAccessory
{
    ...
    public override void Use(Player player, int inventoryIndex)
    {
        ...
        if (inventoryIndex == -1)
        {
            GameObject g = Instantiate(buildingList[player.playerModularBuilding.fakeBuildingID.GetComponent<BuildingAccessory>().oldPositioning].buildingObject, player.playerModularBuilding.fakeBuildingID.gameObject.transform.position, Quaternion.identity);
            ModularBuildingManager.singleton.spawnedBuilding = g;
        }
        else
        {
            GameObject searched = player.playerModularBuilding.FindNearestFence();

            if (searched != null)
            {
                GameObject g = Instantiate(buildingList[0].buildingObject, new Vector3(searched.transform.position.x, searched.transform.position.y, 0.0f), Quaternion.identity);
                ModularBuildingManager.singleton.spawnedBuilding = g;
        ...
        ModularBuildingManager.singleton.AbleFencePositioning();
        ModularBuildingManager.singleton.scriptableFence = this;
    }
    ...
    static Dictionary<int, ScriptableFence> cache;
    public static Dictionary<int, ScriptableFence> dict
    {
        get
        {
            // not loaded yet?
            if (cache == null)
            {
                ...
                ScriptableFence[] items = Resources.LoadAll<ScriptableFence>("");
