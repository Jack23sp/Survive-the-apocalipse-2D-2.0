...
{
    [Command]
    public void CmdAddToLibrary(int inventorySlots, int warehouseSlot, NetworkIdentity identity)
    {
        Library library = identity.GetComponent<Library>();
        if (inventorySlots > -1)
        {
            if (inventory.slots[inventorySlots].amount == 0) return;

            int add = library.Add(inventory.slots[inventorySlots].item, inventory.slots[inventorySlots].amount);
        ...
        if (warehouseSlot > -1)
        {
            if (library.slots[warehouseSlot].amount == 0) return;
            int add = inventory.AddNew(library.slots[warehouseSlot].item, library.slots[warehouseSlot].amount);

            if (add == -10000)
                library.slots[warehouseSlot] = new ItemSlot();
            else
            {
                ItemSlot slot = library.slots[warehouseSlot];
                slot.amount = add;
                library.slots[warehouseSlot] = slot;
            }
        }
    }
...
public partial class Database
{
    ...
    public void SaveLibrary(int index)
    {
        for (int i = 0; i < ((Library)ModularBuildingManager.singleton.buildingAccessories[index]).slots.Count; i++)
        {
            ItemSlot slot = ((Library)ModularBuildingManager.singleton.buildingAccessories[index]).slots[i];
            if (slot.amount > 0)
            {
                connection.InsertOrReplace(new library_item
                {
                    myIndex = index,
                    character = "",
                    slot = i,
    ...
    public void LoadLibrary(int index, Library library)
    {
        for (int i = 0; i < library.amount; i++)
            library.slots.Add(new ItemSlot());

        foreach (library_item row in connection.Query<library_item>("SELECT * FROM library_item WHERE myIndex=?", index))
        {
            if (ScriptableBook.dict.TryGetValue(row.name.GetStableHashCode(), out ScriptableBook itemData))
            {
                Item item = new Item(itemData);
                library.slots[row.slot] = new ItemSlot(item, row.amount);
            }
        }
    }
...
public class Library : BuildingAccessory
{
    ...
    new void Start()
    {
        ...
        {
            if (!ModularBuildingManager.singleton.libraries.Contains(this)) ModularBuildingManager.singleton.libraries.Add(this);
        }
    }
    ...
    public new void OnDestroy()
    {
        ...
        if (isServer || isClient)
        {
            if (ModularBuildingManager.singleton.libraries.Contains(this)) ModularBuildingManager.singleton.libraries.Remove(this);
        }
    }
    ...
    public override void OnStartClient()
    {
        ...
        slots.Callback += OnLibraryInventoryChanged;
    }
