...
public class PlayerSkills : Skills
{
    ...
    [HideInInspector] public Vector2 _currentSkillDirection = Vector2.down;
    protected override Vector2 currentSkillDirection => player == null ? _currentSkillDirection : player.lookDirection;
    ...
    {
        // validate
        if (player.playerNavMeshMovement2D.GetVelocity() != Vector2.zero) return;
    ...
    public void TryUse(int skillIndex, bool ignoreState = false)
    {
        ...
        if (entity.state != "CASTING" || ignoreState)
        {
            ...
            if (CastCheckSelf(skill) && CastCheckTarget(skill))
            {
                ...
                {
                    if (Player.localPlayer.playerMove.states.Contains("AIM"))
                    {
                        Player.localPlayer.playerMove.CmdSetState("SHOOT", new string[1] { "AIM" });
                    }
                    else
                    {
                        if (!Player.localPlayer.playerMove.states.Contains("SHOOT")) Player.localPlayer.playerMove.CmdSetState("SHOOT", new string[0] { });
                    }
                    // cast
                ...
                else
                {
                    ...
                    if (skill.castRange > 0.0f)
                    {
                        ...
                        if (Player.localPlayer.playerMove.states.Contains("AIM") || Player.localPlayer.playerMove.states.Contains("SHOOT")) Player.localPlayer.playerMove.CmdSetState("", new string[2] { "AIM", "SHOOT" });
                    }
