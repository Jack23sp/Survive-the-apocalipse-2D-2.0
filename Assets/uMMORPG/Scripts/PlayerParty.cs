...
public partial class PlayerParty : NetworkBehaviour
{
    ...
    // parties!
    [Header("Party")]
    [SyncVar(hook =nameof(CheckParty))] public Party party; // TODO SyncToOwner later
    ...
    public override void OnStartLocalPlayer()
    {
        ...
        player.playerCallback.PartyChanged(party, party);
    }

    public void CheckParty(Party oldParty, Party newParty)
    {
        if (!player.playerCallback) 
            player.playerCallback = player.GetComponent<PlayerCallback>();
        player.playerCallback.PartyChanged(oldParty,newParty);
    }
