...
public partial class Monster : Entity
{
    ...
    protected override void Start()
    {
        ...
        GetComponent<DamagableObject>().zombie = this;
    }
    ...
    string UpdateServer_DEAD()
    {
        ...
        if (EventDeathTimeElapsed())
        {
            ...
            if (!spawned)
            {
                ...
                {
                    int index = i;
                    loot = Instantiate(ResourceManager.singleton.objectDrop.gameObject, transform.position, Quaternion.identity);
                    loot.GetComponent<CurvedMovement>().startEntity = transform;
                    loot.GetComponent<CurvedMovement>().SpawnAtPosition(inventory.slots[index].item, inventory.slots[index].amount, index, inventory.slots[index].amount);
                }
                if (gold > 0)
                {
                    loot = Instantiate(ResourceManager.singleton.objectDrop.gameObject, transform.position, Quaternion.identity);
                    loot.GetComponent<CurvedMovement>().startEntity = transform;
                    loot.GetComponent<CurvedMovement>().SpawnAtPosition(new Item(), -1, -1, -1, gold);
                }
                spawned = true;
    ...
    {
        animator.enabled = spriteRenderer.enabled;
    }
    ...
    [ServerCallback]
    public void OnAggroEscape(Entity entity)
    {
        if (isNeutral)
        {
            if (entity is Player)
            {
                ...
                //oldDistance = Vector3.Distance(entity.transform.position, transform.position);
                //Vector2 circle2D = Random.insideUnitCircle * (moveDistance * 1.5f);
                //    while (Vector2.Distance(circle2D, entity.transform.position) < oldDistance)
                //    {
                //        circle2D = Random.insideUnitCircle * moveDistance;
                //    }
                //movement.Navigate(new Vector2(transform.position.x, transform.position.y) + circle2D, 0);
                //_state = "MOVING";
                Vector2 playerDirection = ((Vector2)entity.transform.position - (Vector2)transform.position).normalized;
                float angle = Mathf.Atan2(playerDirection.y, playerDirection.x) * Mathf.Rad2Deg;
                float randomAngle = angle + 180f + Random.Range(-90f, 90f); // Aggiungiamo 180 gradi e un angolo casuale nell'intervallo [-90, 90] per ottenere una posizione nel semicerchio opposto
                Vector2 escapeDirection = new Vector2(Mathf.Cos(randomAngle * Mathf.Deg2Rad), Mathf.Sin(randomAngle * Mathf.Deg2Rad));
                Vector2 escapePoint = (Vector2)transform.position + escapeDirection * (moveDistance * 1.5f);
                movement.Navigate(escapePoint, 0);
                _state = "MOVING";
            }
        }
    }
    ...
    public override bool CanAttack(Entity entity)
    {
        if (entity is Player)
        {
            if (((Player)entity).playerMove.states.Contains("SNEAK"))
            {
                if (((Player)entity).playerBlood.current == 0) return base.CanAttack(entity);
                else
                    return false;
            }
            else
