...
public static partial class GuildSystem
{
    ...
    public static void BroadcastChanges(Guild guild)
    {
    ...
    public static bool CreateGuild(string creator, int creatorLevel, string guildName)
    {
            ...
            !Database.singleton.GuildExists(guildName)) // db check only on server, no Guild.CanCreate function because client has no DB.
        {
            ...
            {
                if(ModularBuildingManager.singleton.buildingAccessories[i].owner == creator)
                    ModularBuildingManager.singleton.buildingAccessories[i].group = guildName;
            }

            for (int i = 0; i < ModularBuildingManager.singleton.combinedModulars.Count; i++)
            {
                if (ModularBuildingManager.singleton.combinedModulars[i].owner == creator)
                    ModularBuildingManager.singleton.combinedModulars[i].group = guildName;
            }
            return true;
    ...
    public static void LeaveGuild(string guildName, string member)
    {
            ...
            guild.CanLeave(member))
        {
            ...
            if (Player.onlinePlayers.TryGetValue(member, out Player leavingPLayer))
            {
                for (int e = 0; e < leavingPLayer.playerAlliance.guildAlly.Count; e++)
                {
                    leavingPLayer.playerAlliance.guildAlly.Remove(guildName);
                }
            }
        }
    }
    ...
    public static void TerminateGuild(string guildName, string requester)
    {
        ...
        {
            //if (Player.onlinePlayers.TryGetValue(requester, out Player master))
            //{
            //    for (int i = 0; i < master.playerAlliance.guildAlly.Count; i++)
            //    {
            //        if (guilds.TryGetValue(master.playerAlliance.guildAlly[i], out Guild allyG))
            //        {
            //            for (int e = 0; e < allyG.members.Length; e++)
            //            {
            //                if (Player.onlinePlayers.TryGetValue(allyG.members[e].name, out Player memb))
            //                {
            //                    if (memb.playerAlliance.guildAlly.Contains(guildName)) memb.playerAlliance.guildAlly.Remove(guildName);
            //                }
            //            }
            //        }
            //    }
            ...
            //    for (int i = 0; i < master.playerAlliance.guildAlly.Count; i++)
            //    {
            //        master.playerAlliance.guildAlly.RemoveAt(i);
            //    }
            //}
            ...
            for (int i = 0; i < ModularBuildingManager.singleton.buildingAccessories.Count; i++)
            {
                if (ModularBuildingManager.singleton.buildingAccessories[i].group == guildName)
                {
                    ModularBuildingManager.singleton.buildingAccessories[i].group = string.Empty;
                    ModularBuildingManager.singleton.buildingAccessories[i].owner = requester;
                }
            }

            for (int i = 0; i < ModularBuildingManager.singleton.combinedModulars.Count; i++)
            {
                if (ModularBuildingManager.singleton.combinedModulars[i].group == guildName)
                {
                    ModularBuildingManager.singleton.combinedModulars[i].group = string.Empty;
                    ModularBuildingManager.singleton.combinedModulars[i].owner = requester;
                }
            }
        }
    }
