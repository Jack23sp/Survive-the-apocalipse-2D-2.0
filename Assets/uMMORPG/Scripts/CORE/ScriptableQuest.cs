...
public abstract class ScriptableQuest : ScriptableObject
{
    ...
    public string successive; // this quest has to be completed first
    ...
    // events to hook into /////////////////////////////////////////////////////
    public virtual void OnKilled(Player player, int questIndex, string victim) {}
    public virtual void OnLocation(Player player, int questIndex, Collider2D location) {}
    public virtual void OnGathered(Player player, int questIndex, string objectName, int amount) { }
    public virtual void OnCraft(Player player, int questIndex, string objectName, int amount) { }
    public virtual void OnBuild(Player player, int questIndex, string objectName, int amount) { }
    public virtual void OnPick(Player player, int questIndex, string objectName, int amount) { }

    // fulfillment /////////////////////////////////////////////////////////////
    // we pass the Quest instead of an index for ease of use and because we are
    // read-only here anyway
    public abstract bool IsFulfilled(Player player, int questIndex);

    // OnComplete is called when the quest is completed at the npc.
    // -> can be used to remove quest items from the inventory, etc.
    public virtual void OnCompleted(Player player, int questIndex) {}
