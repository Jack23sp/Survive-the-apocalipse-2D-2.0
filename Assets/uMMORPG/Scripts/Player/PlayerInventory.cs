...
public class PlayerInventory : Inventory
{
    ...
    [SyncVar] public int size = 30;
    [SyncVar] public int abilitySize = 0;
    [SyncVar] public int bagsize = 0;
    ...
    public override void OnStartServer()
    {
        ...
        player.playerWeight.Assign();
        float weight = 0;
        for (int i = 0; i < slots.Count; i++)
        {
            if (slots[i].amount > 0)
                weight += slots[i].item.weight * slots[i].amount;
        }
        player.playerWeight.current += weight;

        Invoke(nameof(InventoryUnsanity), CoroutineManager.singleton.unsanityInvoke);
    }


    public void InventoryUnsanity()
    {
        for (int i = 0; i < slots.Count; i++)
        {
            if (slots[i].amount == 0) continue;

            if (slots[i].item.data.maxUnsanity > 0 && slots[i].item.currentUnsanity > 0)
            {
    ...
    {
        base.OnStopServer();
        CancelInvoke(nameof(InventoryUnsanity));
    }
    ...
    [Command]
    public void CmdInventoryMergeItem(int fromIndex, int toIndex)
    {
        if (InventoryOperationsAllowed() &&
            0 <= fromIndex && fromIndex < slots.Count &&
            0 <= toIndex && toIndex < slots.Count &&
            fromIndex != toIndex)
        {
            ...
            if (slotFrom.amount > 0 && slotTo.amount > 0)
            {
                // make sure that items are the same type
                // note: .Equals because name AND dynamic variables matter (petLevel etc.)
                if (slotFrom.item.name == slotTo.item.name)
                {
                    // merge from -> to
                    // put as many as possible into 'To' slot
                    int put = slotTo.IncreaseAmount(slotFrom.amount);
